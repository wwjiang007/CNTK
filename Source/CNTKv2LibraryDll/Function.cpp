...
namespace CNTK
{
    ...
    FunctionPtr MeanVarianceNormalization(const Variable& operand, const bool useStatsAcrossChannels, const bool doVarianceScaling, float epsilon, const std::wstring& name)
    {
        ...
        else
        {
            return AsBlock(std::move(ElementDivide(operandMeanRemoved, 
                Plus(Sqrt(ReduceMean(Square(operandMeanRemoved), axesToReduce)), 
                    Constant::Scalar(operand.GetDataType(), epsilon)))),
